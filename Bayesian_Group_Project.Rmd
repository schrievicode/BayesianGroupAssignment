...
################################################################################################
#-- Model 2: Random intercept for Region | Priors based on EDA & Student distribution (Fynn) --#
################################################################################################
...
# Model 2: Random Intercept for Region with informed priors & Student's T distribution (Fynn)
...
Upon repeatedly running the cross-validation, we find that there is barely any difference between all five models, indicating that there are different viable approaches to our topic. Furthermore, the standard deviation of the models indicate the uncertainty of the predictions. As the predicted difference in performance of all models fall within two times the standard deviation, no clear conclusions can be made about which model performs best when only looking at these results. Although the positions among the models change, we decided that Model 3 is currently most likely to be the best model for the following reasons:

We found that model 2 & 5 appear to be the most volatile, radically changing their position among different runs of the cross-validation. For model 2, this could indicate that some of the priors may have been set too narrow, leading to it sometimes fitting the model rather well, and sometimes fitting the model poorly, both by chance. For model 5, it could indicate that varying slopes models are too complex for this subject, and are thus sometimes running a risk of overfitting. This is supported by the fact that the other varying slopes model, model 4 is also commonly among the worse models.

Lastly, we find that model 1 and model 3 consistently perform decently well, always being the best or reasonably close to the best model. As such, one can argue that regularization through either Gelman or Horseshoe priors is for the most part sufficient, while increasing complexity (i.e., through the introduction of more narrow priors, or varying slopes models) seem to not meaningfully increase model performance, in the worst case even decreasing it. As such keeping things simple through basic Horseshoe or Gelman priors for a random intercept model seems to be the best option for this dataset and topic. Between the two, model 3 was most commonly able to attain top performance, while model 1 was usually a bit more volatile, making model 3 our model of choice.
