...
Upon repeatedly running the cross-validation, we find that there is barely any difference between Models 1-4 in terms of performance, indicating that there are different viable approaches to our topic. Furthermore, the standard deviation of the models 1-4 indicate the uncertainty of the predictions. As the predicted difference in performance of model 1, 3 and 4 fall within two times the standard deviation, no clear conclusions can be made about which model performs best when only looking at these results. Although the positions among the first 4 models change, we decided that Models 1&3 would be the best option in a real-world application sense, for reason described in 6. below.
